...
+(void)createRequestForImageAndGetResponse:(NSString *) strUrl methodType:(HM_HTTPMethod) method andHeaderDict:(NSDictionary *) dictHeader andParameterDict:(NSDictionary *) dictParameters andImageNameAsKeyAndImageAsItsValue:(NSDictionary<NSString *, UIImage *> *) dictImages imgName:(NSString *) IMG onCompletion:(HMRequestCompletion) completed{
    NSMutableURLRequest * request = [HMWebService createRequestForImage:strUrl methodType:method andHeaderDict:dictHeader andParameterDict:dictParameters andImageNameAsKeyAndImageAsItsValue:dictImages STRSourabh:IMG];
    [HMWebService getResponseFromUrl:request onCompletion:completed];
}
    ...
    [[session dataTaskWithRequest:request completionHandler:^(NSData * data, NSURLResponse * response, NSError * error) {
        ...
        if (error == nil) {
            if (data.length > 0 && httpResponse.statusCode == 200) {
                ...
                if ([HMWebService isObject:result string_1_Array_2_Dictionary_3_Integer_4:3] == YES) {
...
+ (NSMutableURLRequest *)createRequestForImage:(NSString *) strUrl methodType:(HM_HTTPMethod) method andHeaderDict:(NSDictionary *) dictHeader andParameterDict:(NSDictionary *) dictParameters andImageNameAsKeyAndImageAsItsValue:(NSDictionary<NSString *, UIImage *> *) dictImages STRSourabh:(NSString*) str {
    NSString * strApi = [strUrl stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
...
+ (BOOL)isObject:(id) ob string_1_Array_2_Dictionary_3_Integer_4:(int) which {
    @try {
        if (ob) {
            if (which == 1) {
