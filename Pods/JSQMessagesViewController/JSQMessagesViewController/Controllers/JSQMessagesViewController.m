...
- (UICollectionViewCell *)collectionView:(JSQMessagesCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *file_path = @"";
    NSString *strMSG = [NSString stringWithFormat:@"%@",[messageItem text]];
    NSData *data = [[messageItem text] dataUsingEncoding:NSUTF8StringEncoding];
    if ([data length] > 0){
        NSDictionary *json = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
        if([json objectForKey:@"msg"] != nil){
            strMSG = [NSString stringWithFormat:@"%@",[json objectForKey:@"msg"]];
        }
        if([json objectForKey:@"file_path"] != nil){ // Mohan
            file_path = [NSString stringWithFormat:@"%@",[json objectForKey:@"file_path"]];
        }
    ...
    if (!isMediaMessage) {        
        ...
        if (![file_path isEqualToString:@""]){
            cell.mediaView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"profile.png"]];
        }
        if ([UIDevice jsq_isCurrentDeviceBeforeiOS8]) {
  ...
  //  cell.messageBubbleTopLabel.text = @"Testing for minor";
    
    cell.cellBottomLabel.attributedText = [collectionView.dataSource collectionView:collectionView attributedTextForCellBottomLabelAtIndexPath:indexPath];
//cell.cellBottomLabel.text =   @"Testing for demo";
